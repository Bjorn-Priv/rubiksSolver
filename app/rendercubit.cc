...
//array containing float values for all colours
...
void RenderCubit::createXWall(Colour col, float offset, int &Vindex, int &Iindex) {
  ...
  for (int i = 0; i < 2; i++) { //for all Y's
    for (int j = 0; j < 2; j++) { //for all Z's
      data[Vindex] = vertexX;
  ...
  //calculate index to use for triangle drawing
...
void RenderCubit::createZWall(Colour col, float offset, int &Vindex, int &Iindex) {
  ...
  for (int i = 0; i < 2; i++) { //for all possible X's
    for (int j = 0; j < 2; j++) { //for all possible Y's
      data[Vindex] = vertexX[i];
...
void RenderCubit::update(float angle, glm::vec3 center, glm::vec3 axis) {
  for (int i = 0; i < NVERTICES_PER_CUBIT; i++) { //for all floats
    int index = i*NFLOATS_PER_VERTEX;
    //grab location of vertex
